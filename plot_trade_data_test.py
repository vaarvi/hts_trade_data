...
master_df = pd.read_csv("Master_Import_Data.csv")
...
def update_graph(selected_hts, selected_metrics):
    fig = go.Figure()
    selected_hts_str = [str(hts) for hts in selected_hts]

    for hts in selected_hts:
        df_filtered = master_df[master_df["HTS"] == int(hts)]
        ...
        for i, metric in enumerate(selected_metrics):
            ...
            y_values = df_metric[metric].abs()
            if i == 1:  # Apply negative direction for second metric
                y_values = -y_values
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    hovertemplate=f"Country: {{x}}<br>{metric}: {{y}}<extra></extra>",
                )
            )
    ...
    if len(selected_metrics) == 1:
        ...
        y_title = f"{selected_metrics[0]} (↑) & {selected_metrics[1]} (↓)"
    else:
        y_title = "Value"
